...
st.set_page_config(page_title="MusicCip", page_icon="🎵", layout="centered")

st.title("🎵 MusicCip")
st.caption("AI Pembuat Musik, Not Angka & Chord Otomatis")
st.write("Selamat datang di MusicCip! Ciptakan lirik lagu, not angka, dan chord gitar instan.")
...
try:
    ...
    st.error("API Key Gemini belum diatur di Advanced Settings -> Secrets!")
    st.stop()
...
with tab1:
    ...
    tombol_analisis = st.button("Bedah Musik Sekarang", key="btn_analisis")
    ...
    if tombol_analisis:
        if input_musik.strip() == "":
            ...
            with st.spinner("AI sedang membedah struktur musikmu..."):
                try:
                    # Menggunakan model paling standar & didukung penuh saat ini
...
with tab2:
    ...
    tombol_cipta = st.button("Ciptakan Lagu Sekarang", key="btn_cipta")
